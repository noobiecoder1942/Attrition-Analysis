...
#Checking for duplicates in critical fields
sum(duplicated(general_data$EmployeeID))    #Returns 0
sum(duplicated(employee_survey$EmployeeID)) #Returns 0
sum(duplicated(manager_survey$EmployeeID))  #Returns 0
sum(duplicated(in_time$EmployeeID))         #Returns 0
sum(duplicated(out_time$EmployeeID))        #Returns 0

#checking for NA's in different datasets
sum(is.na(general_data)) # returns 28 
sum(is.na(employee_survey)) # returns 83 
sum(is.na(manager_survey))  #returns 0
...
sum(is.na(general_data)) # 16 can be removed

###################################################################################################################
#Data manipulation for IN & OUT time for employees 
#Calculating average working hours for each employee for the previous year
###################################################################################################################
...
#since both have same number of NA's, which shows in-out times are the same. So we can remove NA's from the dataset
in_time       <- in_time[ (!is.na(in_time$in_day_time)), ]
out_time      <- out_time[ (!is.na(out_time$out_day_time)), ]

#Combining in and out time data into a single dataset
...
#Calculating working hours which is the difference between in and out time for a particular date
...
################################################################################################################
setdiff(general_data$EmployeeID, emp_avg$EmployeeID)          #All are identical with no difference in employee id
setdiff(general_data$EmployeeID, employee_survey$EmployeeID)  #All are identical with no difference in employee id
setdiff(general_data$EmployeeID, manager_survey$EmployeeID)   #All are identical with no difference in employee id
...
sapply(base_data, function(x) length(which(is.na(x)))) #no NAs found 

sapply(base_data, function(x) length(which(x == ""))) #No blank spaces found

#######################################################################################################################
#Overall plotting of data on the basis of attrition rate
#######################################################################################################################
...
plot_attrition_fig
#it is seen that 697 employees left the compant in 2015
##################################################################################################################
...
plot_grid(ggplot(base_data, aes(DistanceFromHome))+ geom_histogram(binwidth = 20),
          ...
          align = "v",ncol = 1) 
#No outliers are visible in the above numeric variables
...
#few outliers in  MonthlyIncome, YearsAtCompany, YearsSinceLastPromotion, which is quite possible considering,hence they can be ignoring

##################################################################################################################
#creating new field to understand whether employee is doing extended working hours or not
#As the standard working hours is 8 as is seen from the data, those who have an average working hours of more than 8 can be considered 
#to be doing extended working hours
##################################################################################################################
...
#Conclusion: it is seen that there is a large volume if people working extended hours who 
#tend to leave the company.

##################################################################################################################
#JobTenure: People who have a tendency of changing jobs frequently appear to leave the company within short
#periods of time. This can be found out from Jobtenure field.
##################################################################################################################
...
##################################################################################################################
#as everyone is above the age of 18 this column will not bring any additional information to the model
#so we can drop the age variable from overall analysis, similarly we have other variables such as employee count
...
sum(dummies$PerformanceRating) #Returns 663 which is the number of records with rating 4,represneted as 1, so for rating 3 it is 0
sum(dummies$extended_working_hrs) #Returns 1278 so extended working hours is reprensented by 1 otherwise as 0
...
# Findings: Performance Rating & PercentageSalaryHike is highly correlated so does yearswithcurrrnmanager
...
#predicting probabilities of Attrition for test data
...
test$prob <- test_pred
View(test)
# Let's assume the probability cutoff of 50%.
...
test_conf2 <- caret::confusionMatrix(test_pred_Attrition_1, test_actual_Attrition, positive = "Yes")
test_conf2

#At 30%
test_pred_Attrition_2 <- factor(ifelse(test_pred >= 0.30, "Yes", "No")) 
test_conf3 <- caret::confusionMatrix(test_pred_Attrition_2, test_actual_Attrition, positive = "Yes")
test_conf3

#At 25%
test_pred_Attrition_3 <- factor(ifelse(test_pred >= 0.25, "Yes", "No")) 
test_conf4 <- caret::confusionMatrix(test_pred_Attrition_3, test_actual_Attrition, positive = "Yes")
test_conf4
################################################################################################################
...
################################################################################################################
#Conclusion:
# When the Number of companies worked by a person is more they tend to quit more
...
#As the age increases the people are less tend to quit, which shows they more comfortable with the work environment
#People with more years of experience are less likely to leave the company. 
#Environment Satisfaction, Job Satisfaction are some of the main features that need to be taken into consideration for retaining
#employees
